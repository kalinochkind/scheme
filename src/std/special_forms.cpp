...
static std::pair<std::shared_ptr<SchemeObject>, bool>
...
std::unordered_map<std::string, std::function<std::shared_ptr<SchemeObject>(const std::list<std::shared_ptr<ASTNode>> &,
        ...
        {"read",            [](const std::list<std::shared_ptr<ASTNode>> &, Context &context,
                               std::shared_ptr<SchemeFunc>) {
            try
            {
                return do_quote(readObject(std::cin), context, 0).first;
            }
            catch(end_of_input)
            {
